...
- **Native iOS app: BUILT — lives in `ios-native/` (the `plan-ios-swift.md`
  ...
  **generated by running the real JS modules**
  ...
  `ios-native/project.yml`, iOS 16+, no SwiftData by design) ships Tracker /
...
npm test             # vitest run — 433 tests across 47 files, all green
...
- **Tests:** `npm test` runs **433 vitest tests across 47 files**, all passing.
  ...
  `SpeakerButton`, `PronunciationPractice`, `UiText`, `useProgress`,
  `useJournal`. New non-trivial logic
